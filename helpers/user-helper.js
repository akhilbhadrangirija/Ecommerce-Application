...
module.exports={
 ...
 addAdmin: async (users,callback)=>{
  ...
  var myPlaintextPassword =users.password;
     ...
     // console.log(users);
     db.database().collection(collections.ADMIN_DATA).insertOne(users).then((data)=>{
       // console.log(data);
       })
...
getAdmin:(userData,callback)=>{
  ...
  return new Promise(async(resolve,reject)=>{
      let loginStatus=false
      let respose={}
    let user= await db.database().collection(collections.ADMIN_DATA).findOne({email:userData.email})
    if(user){
        // console.log("user here");
